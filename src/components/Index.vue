...
async function getBeers(page) {
    ...
    if (firstSearch.value) {
        ...
        router.push({
            name: 'index',
            query: { page: page, name: beerName.value, ibu: ibuFilterValue.value, filter: ibuFilterType.value },
            replace: true,
        });
    }
                    ...
                    <ul class="navbar-nav d-flex justify-content-center w-100">
                        <div
                            ...
                            <div class="col-12 col-md-3 ps-0 pe-0 mb-3 mb-md-0">
                                <input class="form-control" type="text" placeholder="Name" v-model.trim="beerName">
                            </div>
                            <div class="col-12 col-md-3 ps-0 ps-md-3 col-lg-2 pe-0 mb-3 mb-md-0">
                                <div class="input-group">
                                    <input class="form-control w-50" type="number" placeholder="IBU" step="0.5" min="0"
                                        max="100" v-model.number="ibuFilterValue">
                            ...
                            <div class="col-6 ps-0 ps-md-3 col-md-auto pe-0">
                                <button class="btn btn-success w-100" type="submit" v-on:click="getBeers(1)"
                                    :disabled="loadingInProgress">Search</button>
                            </div>
                            <div class="col-6 ps-sm-3 col-md-auto pe-0">
                                <button class="btn btn-secondary w-100" type="button" v-on:click="reset()"
                                    :disabled="loadingInProgress">Reset</button>
                            </div>
                        </div>
                    </ul>
        ...
        <main class="mt-4">
            <div class="container">
                ...
                <div class="row row-cols-1 row-cols-sm-2 row-cols-lg-3 g-4">
                    <div v-if="loadingInProgress || errorMessage || noBeersFound" class="col"
                        v-html="loadingInProgress ? 'Please wait...' : noBeersFound ? 'No beers found' : errorMessage">
                    </div>
                    <div v-else class="col" v-for="beer in beers.items" :key="beer.id">
                        <router-link class="text-decoration-none link-dark"
                            :to="{ name: 'beerDetails', params: { beer: beer } }">
                            <BeerItem :beer="beer" />
                        </router-link>
